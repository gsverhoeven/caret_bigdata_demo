...
Todo: split sample in ten equal sized parts outside of train.
This makes sure that the ten "samples" from the big data set are uncorrelated.
Also understand behavior at low sample sizes.

ranger throws error:
"There were missing values in resampled performance measures."

With 20 samples, and removal of near zero variance predictors, we do not expect problems with predicting on the test fold.
This likely happens when the data point with the "good" label is in test.
The ranger model then fails?
...
```
For sample 3, only one datapoint has label "good".
# Plot result
